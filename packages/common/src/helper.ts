  ...
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onloadend = () => {
      if ((reader.result as string).startsWith('data:application/pdf;')) {
        ...
        reject(Error('[@pmee/common] template.basePdf must be pdf data.'));
      }
    };
...
Check this document: https://pdfme.com/docs/custom-fonts#about-font-type`
  ...
  if (fallbackFontNum > 1) {
    throw Error(
      `[@pmee/common] ${fallbackFontNum} fallback flags found in font. true fallback flag must be only one.
Check this document: https://pdfme.com/docs/custom-fonts#about-font-type`
    );
  }
  ...
  if (fontNamesInSchemas.some((f) => !fontNames.includes(f))) {
    throw Error(
      `[@pmee/common] ${fontNamesInSchemas
...
--------------------------`
      ...
      throw Error(`[@pmee/common] Invalid argument:
--------------------------
${message}`);
    }
  }
