...
These can be loaded as plugins from the `@pmee/schemas` package.

You can also create your own schemas and load them similarly as plugins.
This page explains how to use schemas from `@pmee/schemas` and how to create your own.

## Using Schemas from @pmee/schemas

Here, we explain how to import image and QR code schemas from `@pmee/schemas`.

First, install `@pmee/schemas`.

```bash
npm install @pmee/schemas
```

Next, import the required schemas from `@pmee/schemas` to `@pmee/generator` and `@pmee/ui`.

The following code shows an example of importing QR code and image schemas from `@pmee/generator` and `@pmee/ui`.

```ts
import type { Template } from '@pmee/common';
import { text, image, barcodes } from '@pmee/schemas';
...
In this `@pmee/ui` example, we're using the Designer, but you can load plugins in the Form and Viewer in the same way.
...
- **pdf**: Used in `@pmee/generator`, it includes code for rendering schemas into PDFs. The PDF rendering process is handled by [pdf-lib](https://pdf-lib.js.org/).
- **ui**: Used in `@pmee/ui`, it includes code for rendering schemas into the DOM. The ui has the following modes:
...
- **propPanel**: Used in `@pmee/ui`'s [Designer](/docs/getting-started#designer), it allows you to add custom property editing forms to the sidebar when a field is selected. You can fill it out using [form-render](https://xrender.fun/form-render)'s JSON format (widget extensions are also possible).
...
### Learning How to Create from @pmee/schemas' Code

If you're looking to create your own schema, it is recommended to refer to the existing code within `@pmee/schemas` while doing so.  
